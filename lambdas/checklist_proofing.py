...
def get_model_lifecycle_safe():
    ...
    # A couple of quick retries with jitter - Bedrock control-plane can be transiently inconsistent.
    for attempt in range(3):
        try:
            resp = bedrock_control.get_foundation_model(modelIdentifier=MODEL_ID)
...
def build_user_message(question_number, content):
    ...
    if question_number == 5:
        ...
        return (
            "Question 5: Read the Water Systems description and cross-check it against the Water Asset IDs found in the report.\n\n"
            "--- Water Systems Description ---\n"
            f"{desc}\n\n"
            "--- Water Asset IDs Found in Report ---\n"
            f"{', '.join(assets) or 'None found'}\n\n"
            "Rules:\n"
            "1) Consider ONLY these plant types for PASS/FAIL:\n"
            "   - MCW-* = incoming mains cold water\n"
            "   - BCWS-* = boosted cold water storage cisterns / boosted cold water services\n"
            "   - CAL-* = calorifiers\n"
            "   - COMBI-* = combination boilers\n\n"
            "2) Treat wording in the description as equivalent to asset IDs where the meaning is clearly the same. For example:\n"
            "   - “Calorifier 1” = CAL-01\n"
            "   - “calorifiers 2, 3 & 4” = CAL-02, CAL-03, CAL-04\n"
            "   - “2 boosted cold water storage cisterns / tanks” = BCWS-01 and BCWS-02\n"
            "   - “2 mains cold water services” = MCW-01 and MCW-02\n\n"
            "3) IGNORE these when deciding PASS/FAIL:\n"
            "   - showers / spray outlets\n"
            "   - any pumps (e.g. BCWP-*, CALP-*)\n"
            "   - TMVs\n"
            "   - boiler pressurisation units\n"
            "   - fire sprinkler systems\n"
            "   - feed and expansion cisterns\n"
            "   - irrigation systems\n"
            "   - narrative mentions of pumps, hoses, deadlegs, dead ends, bib taps, outlets, or subordinate loops\n\n"
            "4) PASS if:\n"
            "   - every relevant plant type named in the description has a matching asset ID in the asset list, and\n"
            "   - there are no extra relevant plant IDs of the types MCW-*, BCWS-*, CAL-*, COMBI-* that contradict the description.\n\n"
            "5) Do NOT fail just because the description uses words instead of exact IDs.\n\n"
            "Output format rules:\n"
            "   - If it passes, reply with EXACTLY: PASS\n"
            "   - If it fails, reply with EXACTLY one line in this format: FAIL: <specific reason>\n"
            "   - Do not include any other text before or after your answer.\n"
            "   - The fail reason must clearly state what is missing, unmatched, or extra."
        )
...
def send_to_bedrock(user_text):
    ...
    # Caller identity is helpful, but should never break a run
    try:
        sts = boto3.client("sts", config=AWS_RETRY_CONFIG)
        logger.info("STS=%s", sts.get_caller_identity())
    except Exception as e:
        logger.warning("STS caller identity lookup failed: %s", e)

    lifecycle = get_model_lifecycle_safe()
    ...
    for q_num in range(1, 15):
        try:
            ...
            if q_num == 5:
                ...
                # Preserve your existing single-word behaviour, but still enrich FAIL with raw output
                if m and m.group(1).upper() == "PASS":
                    proofing_results[f"Q{q_num}"] = "PASS"
                else:
                    proofing_results[f"Q{q_num}"] = format_pass_fail(
                        ai_reply or "FAIL",
