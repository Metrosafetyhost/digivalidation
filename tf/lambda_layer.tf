...
data "aws_iam_role" "asset_categorisation_role" {
  name = "bedrock-lambda-asset_categorisation"
}

resource "aws_iam_role_policy_attachment" "attach_s3_read_pabiltotesting_to_asset_cat" {
...
resource "aws_iam_role_policy_attachment" "attach_s3_read_metrosafetyprodfiles_to_pdf_qa" {
...
# 3) Look up the existing REST API Gateway by name
#    (CHANGE THIS to your actual API Gateway name)
data "aws_api_gateway_rest_api" "pdf_qa_api" {
  name = "bedrock-pdf-qa-api"
}

# 4) Create a standalone policy that allows API Gateway to invoke this Lambda
#    NOTE: This policy is NOT attached to the Lambda role — it is enforced by Lambda via aws_lambda_permission.
resource "aws_iam_policy" "apigw_invoke_pdf_qa" {
  name        = "ApiGatewayInvokePdfQa"
  description = "Allow API Gateway to invoke bedrock-lambda-pdf_qa"
    ...
    Statement = [
      {
        ...
        Effect = "Allow",
        Action = "lambda:InvokeFunction",
        Resource = data.aws_lambda_function.pdf_qa_function.arn,
        Principal = {
          Service = "apigateway.amazonaws.com"
        },
        Condition = {
          ArnLike = {
            # Allows ANY stage / ANY method / ANY resource for this REST API
            "AWS:SourceArn" = "arn:aws:execute-api:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:${data.aws_api_gateway_rest_api.pdf_qa_api.id}/*/*/*"
          }
        }
      }
    ]
...
# 5) Apply that permission to the Lambda using aws_lambda_permission (this is the actual “invoke permission”)
resource "aws_lambda_permission" "allow_apigw_invoke_pdf_qa" {
  statement_id  = "AllowApiGatewayInvokePdfQa"
  action        = "lambda:InvokeFunction"
  function_name = data.aws_lambda_function.pdf_qa_function.function_name
  principal     = "apigateway.amazonaws.com"

  source_arn = "arn:aws:execute-api:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:${data.aws_api_gateway_rest_api.pdf_qa_api.id}/*/*/*"
}